...
class Game extends entity_1.default {
    constructor(name, player, xWord) {
        ...
        this.xWord = {
            ...xWord,
            grid: (0, shared_1.emptyGrid)(xWord.grid),
        };
        ...
        this.gameState = shared_1.GameState.waitingToStart;
    }
    start() {
        const players = Array.from(this.players.values());
        const allPlayersReady = players.every((player) => player.ready);
        if (allPlayersReady) {
            this.gameState = shared_1.GameState.inProgress;
            return true;
        }
        return false;
    }
    fillPlayerTileBar(playerId) {
        ...
        while (player.tileBar.length < shared_1.TILE_BAR_SIZE) {
            ...
            const randomChar = (0, shared_1.get1Random)(filteredChars);
            // make a new tile with the same char
            player.tileBar.push((0, shared_1.charToTile)(randomChar));
        }
    }
    ...
    playTile({ playerId, tileId, pos, }) {
        ...
        if (this.solvedXWord.grid[row][col].char !== tile.char) {
            playerInfo.score += shared_1.SCORE_DECREASE;
            return false;
        }
        // -- correct play --
        // play the tile
    ...
    checkGameOver() {
        const numEmptyTiles = (0, shared_1.countEmpty)(this.xWord);
        const isGameOver = numEmptyTiles === 0;
        if (isGameOver) {
            this.gameState = shared_1.GameState.complete;
        }
    }
