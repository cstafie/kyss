...
const mapEntries = (entries) => {
    return entries.map((entry) => ({
        ...
        isComplete: false,
        direction: entry.direction === "Across" ? shared_1.Direction.across : shared_1.Direction.down,
    }));
};
const getRandomXWord = () => {
    const path = __dirname + "/assets/generated_xWords/";
    const filePaths = fs.readdirSync(path);
    const randomFile = (0, shared_1.getNRandom)(filePaths, 1)[0];
    const jsonXWord = JSON.parse(fs.readFileSync(path + randomFile).toString());
    return {
        ...jsonXWord,
        grid: (0, shared_1.mapGrid)(jsonXWord.grid),
